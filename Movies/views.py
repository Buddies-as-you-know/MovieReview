...
from .forms import Comment_movie_CreateForm, Comment_tv_CreateForm
...
def search(request):
    ...
    # Render the template
    return render(request, 'Movie/results.html', {
        "data": data.json(),
    ...
    if movies.exists():
        for obj in movies:
            if (float(query_min) <= float(obj.average_stars())) and (float(query_max) >= float(obj.average_stars())):
                ...
                data_movie['Ranking'] = Ranking_movie
...
def view_tv_detail(request, tv_id): 
    ...
    TV(id = tv_id,stars = average).save()
    context = {
            "data": data.json(),
...
def view_movie_detail(request, movie_id):
    ...
    if not(request.user.id == None):
        comments = Comment_movie.objects.filter(movie_id=movie_id).exclude(user=request.user).order_by("-id")
    ...
    Movie(id = movie_id,stars = average).save()
    context = {
        "data": data.json(),
        "recommendations": recommendations.json(),
        "type": "movie",
