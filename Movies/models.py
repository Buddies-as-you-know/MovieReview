...
alphanumeric = RegexValidator(
    r'^[0-9]*$', 'Only alphanumeric characters are allowed.')


class Movie(models.Model):
    id = models.CharField(primary_key=True, editable=False, validators=[alphanumeric], max_length=9999)
    ...
    def average_stars(self):
        ...
        if n_comments:
            self.stars = round(sum([comment.stars for comment in comments]) / n_comments, 3)
        else:
            self.stars = 0
        self.save()
        return self.stars
...
class Comment_tv(models.Model):
    ...
    comment = models.TextField(max_length=1000)
    stars = models.FloatField(blank=False, null=False, default=0, validators=[MinValueValidator(0.0), MaxValueValidator(10.0)])
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE)
