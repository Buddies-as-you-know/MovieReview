...
def Comment_List_Movie(request,user_id):
    comment_movie = Comment_movie.objects.filter(user=user_id)
    context = {}
    context["user"] = CustomUser.objects.filter(id=user_id)
    if comment_movie.exists():
        movie_list = []
        for obj in comment_movie:
            data = requests.get(f"https://api.themoviedb.org/3/movie/{obj.movie.id}?api_key={TMDB_API_KEY}&language=en-US")
...
def Comment_List_TV(request,user_id):
    comment_tv = Comment_tv.objects.filter(user=user_id)
    context = {}
    context["user"] = CustomUser.objects.filter(id=user_id)
    if comment_tv.exists():
        tv_list = []
        for obj in comment_tv:
            data = requests.get(f"https://api.themoviedb.org/3/tv/{obj.tv.id}?api_key={TMDB_API_KEY}&language=en-US")
            data_tv = data.json()
